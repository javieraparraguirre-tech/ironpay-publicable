...
const BOOKING_LINK_FALLBACK = "https://ironbox-reservas.vercel.app/socio";
...
async function saveSettings(event) {
  ...
  const { error } = await supa.from("app_settings").update({
    ...
    booking_link_url: form.booking_link_url.trim(),
...
function renderSettings() {
  ...
  $("#settingsForm [name='booking_link_url']").value = state.settings.booking_link_url || "";
...
function renderBookingPortalLink(url) {
  ...
  if (!link) return;
  const finalUrl = url || BOOKING_LINK_FALLBACK;
  link.href = finalUrl;
...
function setTransferNoticeMessage(message, type = "ok") {
  ...
  if (!box) return;
  box.innerHTML = message ? `${esc(message)}${bookingReturnButtonHtml(type)}` : "";
...
function bookingReturnButtonHtml(type) {
  const url = state.portalData?.settings?.booking_link_url || BOOKING_LINK_FALLBACK;
  if (type !== "ok") return "";
  return `<a class="notice-action" href="${esc(url)}">Volver a reservas</a>`;
}
